...
if ($conexion)
{           
  $resultado = mysqli_query($conexion,"SELECT v.idventa as id, v.idsucursal,
...
foreach ($resultado2 as $column) {
    ...
    $item->setCodProducto($column['COD'])
        ...
        ->setDescripcion(cleanStringSunat($column['nombreProd']))
        ->setMtoBaseIgv($base)
        ->setPorcentajeIgv($porcIgv)
        ->setIgv($igvLinea)
        ->setTipAfeIgv($tipoafecto)
        ->setTotalImpuestos($column['proigv'] == "Gravada" ? $igvLinea : 0)
...
$manuel = $fg->numletras(round($totalVenta, 2));
...
$note
->setMtoOperGravadas(round($opGravada, 2))
->setMtoOperExoneradas(round($opExonerada, 2))
->setMtoOperInafectas(0)
->setMtoIGV(round($igvTotal, 2))
