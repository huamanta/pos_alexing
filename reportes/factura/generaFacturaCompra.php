<?php
	...
	if(empty($_GET["id"]))
	{
		...
		$query = mysqli_query($conexion,"SELECT i.idcompra,i.idproveedor,p.nombre as proveedor,p.direccion,i.tipo_c,p.tipo_documento,p.num_documento,p.email,p.telefono,i.idpersonal,u.nombre as personal,i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,date_format(i.fecha_hora,'%d/%m/%y') as fecha,i.impuesto,i.total_compra, i.monto_gravado as gravada, i.monto_exonerado as exonerada, i.monto_igv as igv FROM compra i INNER JOIN persona p ON i.idproveedor=p.idpersona INNER JOIN personal u ON i.idpersonal=u.idpersonal WHERE i.idcompra='$idventa'");
		...
		if($result > 0){
			...
			ob_start();
		    include(dirname('__FILE__').'/facturaCompra.php');
		    $html = ob_get_clean();
