...
$("#formapago").change(function () {
  if ($(this).val() === "Si") {
    if (detalles === 0) {
      Swal.fire(
        "Alerta",
        "Agrega al menos un producto para habilitar los datos de crédito.",
        "warning",
      );
...
function mostrar(idcotizacion) {
  ...
  $.post(
    "controladores/cotizaciones.php?op=listarDetalle&id=" + idcotizacion,
    function (r) {
      $("#detallesm").html(r);
    },
  );
...
function pintarCotizaciones(data, permissions) {
  let html = "";
  ...
  data.forEach((item) => {
    const url2 = `reportes/factura/generaFacturaCoti.php?id=${item.idcotizacion}`;
    let estado = "";
    let editar = "";
    let desistir = "";
    ...
    html += `
            <tr>
                ...
                <td>S/ ${parseFloat(item.total_venta).toFixed(2)}</td>
                <td>
                    ${estado}
                </td>
                <td>
                    <button class="btn btn-warning btn-xs" onclick="mostrar(${item.idcotizacion})" data-toggle="tooltip" title="" target="blanck" data-original-title="VER"><i class="fa fa-eye"></i></button>
					${editar}
					<a target="_blank" href="${url2}" data-toggle="tooltip" title="" target="blanck" data-original-title="PDF"> <button class="btn btn-info btn-xs"><i class="fa fa-file"></i></button></a>
					<a target="_blank" data-toggle="tooltip" title="" target="blanck" data-original-title="ENVIAR COMPROBANTE"> <button class="btn btn-success btn-xs" onclick="EnviarComprobante(${item.idcotizacion})"><i class="fab fa-whatsapp"></i></button></a>
					${desistir}
                </td>
...
listarCotizaciones = new FluentPaginator({
  ...
  renderTabla: pintarCotizaciones,
  tableBody: "#tbody_cotizaciones",
  extraParams: () => ({
...
function desistir(idcotizacion) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire(
        "! Cancelado ¡",
        "Se Cancelo la anulación de la Cotización",
        "error",
      );
    }
  });
...
function modificarSubtotales() {
  ...
  filas.forEach((fila) => {
    ...
    let precio =
      parseFloat(fila.querySelector('[name="precio_venta[]"]').value) || 0;
