...
function guardaryeditar(e) {
  ...
  if (iddestino == idorigen) {
    Swal.fire(
      "Atención",
      "El almacén destino debe ser distinto al de origen",
      "warning",
    );
    return;
  }
...
function pintarMisSolicitudes(data) {
  ...
  data.forEach((item) => {
    ...
    html += `
            <tr>
                <td>${item.correlativo || 'N/A'}</td>
                <td>${item.origen}</td>
                <td>${item.destino}</td>
                <td>${item.fecha}</td>
                <td>${item.usuario_solicita}</td>
                <td>${item.fecha_aceptacion || '-'}</td>
                <td>${item.usuario_acepta || '-'}</td>
                <td>${item.estado_str}</td>

                <td>
                    <button class="btn btn-info btn-sm" title="Ver solicitud" onclick="verProductosSolicitud(${item.idtraslado})"><i class="fa fa-eye"></i></button> 
                    ${anular}
                    ${imprimir}
                </td>
            </tr>
...
function pintarSolicitudes(data) {
  ...
  data.forEach((item) => {
    ...
    var estado = item.estado !== 'pendiente' ? true : false;
  ...
  data.forEach((item) => {
    ...
    html += `
            <tr>
                ...
                <td>
                    <button class="btn btn-info btn-sm" title="Ver traslado" onclick="verProductosSolicitud(${item.idtraslado}, 'traslado')"><i class="fa fa-eye"></i></button> 
                    ${anular}
                    ${imprimir}
                </td>
            </tr>
...
function listarProductos(busqueda = "", idsucursal) {
  if (!idsucursal) {
    Swal.fire(
      "Atención",
      "Seleccione un almacén destino antes de agregar productos",
      "warning",
    );
    return;
  }
    ...
    function (response) {
      ...
      $.post(
        ...
        function (res) {
          ...
          $("#sucursal_destino_solicitud").val(res.destino || "");
        },
        "json",
      );
      ...
      if (data.soloLectura) {
        ...
        $("#tituloSolicitudLabel").html(`
                    <i class="fa fa-eye"></i> <b>Detalle de ${tipo} #${idtraslado}</b>
                `);
        ...
        $("#tituloSolicitudLabel").html(`
                    <i class="fa fa-check-circle"></i> <b>Aprobar ${tipo} #${idtraslado}</b>
                `);
        ...
        data.productos.forEach((p) => {
          html += `
                    <tr>
                        ...
                        <td>
                            <input type="hidden" class="idProductoHidden" value="${p.idproducto}">
                            <input type="number" class="form-control cantidadProducto" value="${p.cantidad_solicitada}" min="1">
                        </td>
                        <td>
                            <select class="form-control estadoProducto">
                                <option value="pendiente" ${p.estado_detalle === "pendiente" ? "selected" : ""}>Pendiente</option>
